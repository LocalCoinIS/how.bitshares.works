...
Prediction markets are assets that trade freely and can be borrowed from the market at a 1:1 ratio with the backing asset (which could be any other asset, including LLC, USD, GOLD).
...
For example, in the `PM.PRESIDENT2016` if you want to bet on a negative outcome with `100k LLC`, you can borrow `100k PM.PRESIDENT2016` by paying `100k LLC` to the network.

.. note:: Since PM-Assets can technically be pegged by any other asset, you may need to pay USD (or anything else) instead of LLC.
...
If the bet resolves negatively (price feed of `0`), your dellc is worth `debt = amount * price = 0 LLC`, you can reclaim your collateral at zero cost, and get to keep `20%` profits from selling the token at `0.2`. If instead the bet resolves positively and you sold all tokens, you cannot close your borrow position to redeem your collateral. However, your total loss is reduced by `20%` for selling the tokens at the market.
