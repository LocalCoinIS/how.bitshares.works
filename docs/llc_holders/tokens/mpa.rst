...
> A simple example would be a MPA that is backed by USD (a stable crypto token within LocalCoin) that requires a collateral ratio of 200%. Then, in order to get new coin, we can borrow 100 USD worth of new coins by paying 200 USD.
...
But there is more! Everyone that holds your (MPA) coin in LocalCoin can
...
BitAssets can be created and owned by anyone on the network. However, those that are owned by the LocalCoin Committee, are called
...
A *SmartCoin* (synonym for MPA) is a crypto-currency that *always* has 100% or more of its value backed by the LocalCoin core currency (LLC), to which they can be converted at any time, as *collateral* in a CFD.
